# healthEdge AI — Credentialing Status Agent (Demo)

A standalone, static, Vercel-deployable pre-sales demo website showcasing the **Credentialing
Status Agent** — a Salesforce Agentforce **Employee Agent** built for healthcare payer
credentialing teams. Give it a Credential record's `CR-######` Name and it summarizes the
case, explains the current status and stage in plain language, and identifies missing
documents — automatically emailing the provider a reminder when documents are still
outstanding.
...
- **dashboard.html** — Authenticated app shell with sidebar navigation, a mock credentialing
  case and provider profile, and a floating chat-launcher stub where the real Agentforce
  embedded chat will be wired in.
- **assets/css/style.css** — All styling (violet/purple clinical SaaS look, fully responsive).
- **assets/js/data.js** — Mock/demo data (provider profile, credentialing case, case history,
  status explanations, missing documents). No real PHI/PII — all names and values are
  fictional.
...
- **Login gate**: `assets/js/app.js` validates the login form client-side (required
  ...
  and `sessionStorage.heAI_agent = 'CredentialingStatusAgent'` before redirecting to
